...
def convert_to_json_ld(json_data):
    ...
    def create_measurement_parameter(type_, value, unit):
        return {
            "@type": type_,
            "hasNumericalPart": {"@type": "Real", "hasNumericalValue": value},
        ...
        if step["type"] == "ElectricCurrent":
            ...
            task = {
                "@type": (
                    ...
                    else "ConstantCurrentDischarging"
                ),
    ...
    for instruction in json_data.get("instructions", []):
        ...
        top_task = {
            ...
            "hasMeasurementParameter": create_measurement_parameter(
                "IterativeStep", instruction.get("repeat"), "emmo:UnitOne"
            ),
            "hasTask": first_task,
        }
