...
@autocast(current=ontology.Ampere, voltage=ontology.Volt)
def charge(
    current: float,
    ...
    if voltage is None:
        chargeStep = target.generator.add(
            ast.Step(
                ontology.ConstantCurrentCharging(),
                [ast.Setpoint(ontology.ChargingCurrent, current)],
                limits or [],
            )
        )
    else:
        chargeStep = target.generator.add(
            ast.Step(
                ontology.ConstantCurrentConstantVoltageCharging(),
                [
                    ast.Setpoint(ontology.ChargingCurrent, current),
                    ast.Setpoint(ontology.ChargingVoltage, voltage),
                ],
                limits or [],
            )
        )
...
def discharge(
    ...
    if voltage is None:
        dischargeStep = target.generator.add(
            ast.Step(
                ontology.ConstantCurrentDischarging(),
                [ast.Setpoint(ontology.DischargingCurrent, current)],
                limits or [],
            )
        )
    else:
        dischargeStep = target.generator.add(
            ast.Step(
                ontology.ConstantCurrentConstantVoltageDischarging(),
                [
                    ast.Setpoint(ontology.DischargingCurrent, current),
                    ast.Setpoint(ontology.DischargingVoltage, voltage),
                ],
                limits or [],
            )
        )
...
def pause(
    ...
    return target.generator.add(
        ast.Step(
            ...
            [
                ast.Setpoint(
                    ontology.ChargingCurrent, ast.NumericLiteral(0, ontology.Ampere)
                ),
...
class time:
    ...
    def toValue(self) -> ast.Setpoint:
        ...
        return ast.Setpoint(
            ontology.StepDuration, ast.NumericLiteral(value, ontology.Second)
        )

    def toOntology(self):
        return self.toValue().toOntology()
